...
module.exports = class extends Command {
    ...
    constructor(...args) {
        super(...args, {
            ...
            aliases: [],
            category: "Utilities",
            description: "Gives a member DJ role.",
            usage: "<user>",
    ...
    async execute(message, args) {
        ...
        let role = await message.guild.roles.cache.get(`734633779276021810`);

        if (!role) return message.react(`❌`);

        if (args.length < 1) return message.reply(`Please provide a user. Make sure they're **level 5**`);
