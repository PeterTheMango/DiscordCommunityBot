...
class WeeklyChatLeaderboard {
    ...
    async update() {
        ...
        let leaderboardEmbed = new MessageEmbed({
            title: "Weekly Leaderboard [Messages]",
            color: "RANDOM",
            thumbnail: {
    ...
    async save() {
        ...
        await insert.save().catch(err => console.log(`Unable to save new Weekly Chat Leaderboard. Error:\n${err.toString()}`));

        let insertCooldown = new Cooldown(this.message, "weeklychatlb_reset", Date.now() + 604800000);
...
module.exports = WeeklyChatLeaderboard;