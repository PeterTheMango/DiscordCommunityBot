...
module.exports = class extends Command {
    ...
    constructor(...args) {
        super(...args, {
            ...
            aliases: [],
            category: "Moderation",
            description: "Gives a member pic perm's role.",
            usage: "<user>",
    ...
    async execute(message, args) {
        ...
        let role = await message.guild.roles.cache.get(`730459301486919681`);

        if (!role) return message.react(`❌`);

        if (args.length < 1) return message.reply(`Provide a user you would like to give pic perms to. Make sure they're following our <#990965755120091156>`);
